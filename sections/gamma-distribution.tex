...
$\alpha \geq 1$ as shown in Figure~\ref{fig:gd}C. \cc{[Felix: You said: \enquote{I think it's best if you write $\varrho = \dots \approx \dots := \tilde{\varrho}$ This way, you repeat the definition of the original rho, clarify where exactly the approximation enters and properly define rho-tilde as the result of this approximation.} I don't think it's possible to write it like that. The approximation comes from approximating the the distribution of $P_{ij}$ as $\Gamma(\alpha, \beta)$ (instead of $\Gamma^T(\alpha,\beta)$). The definition for $\varrho$ (or any tilde version thereof) doesn't not change and remains $\frac{\E(P_{ij}^2)}{\E(P_{ij})^2}.$ In the current state, I don't feel like $\tilde{\varrho}$ is adding anything and I would prefer going back to the earlier version without it. Specifically I feel that $\tilde{\varrho}$ is misleading, as it implies a similarity to $\varrho$. However, I don't see $\tilde{\varrho}$ as a well defined network measure as it comes from a calculation where connection probabilities $P_{ij}$ can be larger than 1, which obviously makes no sense. Please make a suggestion.]} Earlier comment: \cc{[Felix: you asked me to use $\varrho$ in the equation above, but it's an approximation so don't want to do that. I introduced $\tilde{\varrho}$ instead, I don't nknow if that makes it better]} 
...
\cite{Perin2011}. \cc{[Felix: Daniel, do I have anything to add to
    this specifically? Jochen asked me to talk to you about this.]}
